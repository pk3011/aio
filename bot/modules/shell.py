...
def up(update, context):
    ...
    if len(args) > 1:
        ...
        cmd = f"python3 fup.py --path {shlex.quote(link)}"
...
def gdl(update, context):
    message = update.effective_message
    args = update.message.text.split(" ", maxsplit=1)
    link = ''
    if len(args) == 1:
        return message.reply_text('Send a GDrive link with command to Download from your TD...', parse_mode='HTML')
    if len(args) > 1:
        link = args[1]
        msg = sendMessage("Downloading! Please Wait...", context.bot, update.message)
        cmd = f"python3 gdl.py {shlex.quote(link)}"
    process = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
    stdout, stderr = process.communicate()
    reply = ''
    stdout = stdout.decode()
    ...
    if len(reply) != 0:
        ...
        message.reply_text('This link is not accessible via your email. Check Again..', parse_mode='Markdown')

def g_hl(update, context):
    message = update.effective_message
    args = update.message.text.split(" ", maxsplit=1)
    link = ''
    if len(args) == 1:
    ...
    if len(args) > 1:
        link = args[1]
        cmd = f"bash web/g_hl {shlex.quote(link)}"
    process = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
    stdout, stderr = process.communicate()
    reply = ''
    stdout = stdout.decode()
...
UP_HANDLER = CommandHandler(BotCommands.UpCommand, up,
                               filters=CustomFilters.owner_filter, run_async=True)
GDL_HANDLER = CommandHandler(BotCommands.GDlCommand, gdl,
                               filters=CustomFilters.owner_filter, run_async=True)
G_HANDLER = CommandHandler(BotCommands.GCommand, g_hl,
...
dispatcher.add_handler(GDL_HANDLER)
